...
```{r}
# ATTEMPT to remove all words that are used fewer than X times

# code will still run if you skip this section 

# convert dfm to df in order to run manipulations
# add a row at the bottom for total word counts
test_df <- convert(corpdfm, to = "data.frame") %>%
  bind_rows(colSums(corpdfm, na.rm = TRUE))
test_df[16, 1] <- "Totals"
  
remove_words <- c() # create an empty list of columns to remove (later) 

for (i in 2:ncol(test_df)) { # for each column in the data frame
   if (test_df[16, i] < 20) { # if the total number of words is less than 20
     remove_words <- append(remove_words, colnames(test_df)[i]) # add word to the list of words to be removed
   } 
}

#test_df2 <- select(test_df, -remove_cols) # now make new dataframe without those columns
#test_dfm <- dfm(test_df2) # unable to turn it back into a dfm

colnames(test_df)[1]
# idea: save a list of all words in the for loop and then create a new dfm 
# using the "remove" argument to exclude those columns
# issue: I don't know how to extract the name of the word from the column title

# make new corpdfm WITHOUT the words that are infrequent
...
#test_df <- convert(corpdfm, to = "data.frame")
# corpdfm_filter <- test_df %>%
#   replace(is.na(.), 0) %>%
#   summarise_all(sum)

#bind_rows(test_df, colSums(corpdfm, na.rm=TRUE))
#colSums(corpdfm, na.rm = TRUE)
```

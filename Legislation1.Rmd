...
for (i in 2:ncol(test_df)) { # for each column in the data frame
   if (test_df[16, i] < 10) { # if the total number of occurances is less than 10
...
# because of issues between dataframes and dfm objects)
...
temp_df_flip <- t(test_df)
temp_df_flip <- as.data.frame(temp_df_flip) 
temp_df_flip <- setDT(temp_df_flip, keep.rownames = T)
colnames(temp_df_flip) <- as.character(unlist(temp_df_flip[1,]))
temp_df_flip = temp_df_flip[-1, ]

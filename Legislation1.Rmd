...
```{r}
### This section remove all words that are used fewer than 1 times in either jurisdiction

# can be changed to set a higher threshold
# code will still run if you skip this section

temp_df <- convert(corpdfm, to = "data.frame") 

#total_all_names <- mapply(sum, temp_df[, -1], USE.NAMES = T)

#temp_df_test_sums <- convert(corpdfm, to = "data.frame") %>%
#  bind_rows(total_all_names) # old method


#test_eu <- colSums(corpdfm, n = EU_rows)
...
for (i in 2:ncol(temp_df)) { # for each column in the data frame
   if ((total_eu[i-1] < 1) | (total_us[i-1] < 1)) { # if the total number of occurances is less than once in either jurisdiction
